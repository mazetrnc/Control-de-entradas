...
def asegurar_archivo_carpeta_reportes_config():
    ...
    solo con comentarios) por default: en ese caso los reportes se guardan
    en la carpeta local 'reportes/'. Si se escribe ahí la ruta de una
    carpeta sincronizada (Google Drive, OneDrive, etc.), los reportes se
    guardan directo ahí y esa misma app de sincronización los sube sola a
    la nube — sin que el operador tenga que hacer nada más."""
    if not os.path.exists(CARPETA_REPORTES_CONFIG_PATH):
        contenido = (
            "# Escribe aquí (sin el #) la ruta completa de la carpeta donde quieres\n"
            "# que se guarden los reportes quincenales -- por ejemplo, tu carpeta\n"
            "# sincronizada de Google Drive o OneDrive. Si dejas este archivo vacío\n"
            "# o solo con comentarios, los reportes se guardan en la carpeta local\n"
            "# 'reportes', junto al programa.\n"
            "#\n"
            "# Ejemplo en Windows:\n"
...
def obtener_carpeta_reportes():
    ...
    comentario (#) ni esté vacía, como ruta personalizada para guardar los
    reportes. Si el archivo no existe, está vacío, o la línea está en
    blanco, se usa la carpeta local 'reportes/' por default."""
        ...
        with open(CARPETA_REPORTES_CONFIG_PATH, "r", encoding="utf-8-sig") as f:
            for linea in f:
                ...
                if linea and not linea.startswith("#"):
                    return linea
    return os.path.join(BASE_DIR, 'reportes')
...
def enviar_reporte_por_correo(ruta_pdf, etiqueta_periodo):
    ...
    try:
        ...
        mensaje["Subject"] = f"Reporte de asistencia PAE — {etiqueta_periodo}"
        ...
        logging.info(f"Reporte enviado por correo a: {', '.join(destinatarios)}")
    except Exception:
...
def gestionar_reporte_quincenal():
    ...
    try:
        ...
        ruta_generada = generar_reporte_quincenal(
            DB_PATH, CARPETA_REPORTES, (inicio_anterior, fin_anterior), etiqueta_periodo
        )
        if ruta_generada:
            logging.info(f"Reporte quincenal generado ({etiqueta_periodo}): {ruta_generada}")
            enviar_reporte_por_correo(ruta_generada, etiqueta_periodo)
        with open(ULTIMO_REPORTE_PATH, "w") as f:
            f.write(id_quincena_actual)
    except Exception:
